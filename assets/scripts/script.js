...
const apiKey = "502118d445a07e0aa19f571b11be186e";
    ...
    if(location){
        ...
        try {
            ...
            for(let i = 0; i < data.length; i++)
            {
                ...
                //preventing fetch from returning data for jiberish input like A, B, C, Be, La etc.

                // for New York input New York County
...
//turn timestap to dd/mm/yyyy date
...
function  addToSearchHistory(location){
    ...
    // no empty input &&  no jiberish input && location not saved already
...
searchHistoryArea.addEventListener("click", async function(e){

    e.preventDefault();

    //check which exactly button from search history area was clicked excluding search button
    
    if (e.target.tagName === "BUTTON" && e.target.id!== "search-button"){
